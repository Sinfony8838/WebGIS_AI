...
TEMPLATE_KEYWORDS: List[Tuple[str, List[str], str]] = []
...
class AssistantService:
    ...
    def plan_voice_actions(
        ...
        visual_query_action = self._resolve_visual_query_action(normalized_message, lowered)
        if visual_query_action:
            actions.append(visual_query_action["action"])
            narrative_parts.append(visual_query_action["narrative"])
    ...
    def _resolve_visual_query_action(self, message: str, lowered: str) -> Optional[Dict[str, Any]]:
        if "人口" not in message:
            return None
        rank_intent = any(token in lowered for token in ("top", "top-", "排名")) or any(token in message for token in ("前", "最多", "最高"))
        if not rank_intent:
            return None
        if not any(token in message for token in ("地级市", "城市", "市")):
            return None
        year_match = re.search(r"(20\d{2}|19\d{2})", message)
        limit_match = re.search(r"(?:top|Top|TOP)\s*-?\s*(\d+)|前\s*(\d+)", message)
        ...
        return {
            ...
            "narrative": f"我会生成 {year} 年地级市人口 Top{limit} 高亮图层。",
        }
