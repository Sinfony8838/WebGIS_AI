...
class ResourceAuthorizationMatrixTest(unittest.TestCase):
    ...
    def test_bare_paths_cannot_escape_project_or_builtin_roots(self) -> None:
        workspace = self._worker_sandbox()
        for source in (
            f"../{self.project_b}/data.geojson",
            f"..\\{self.project_b}\\data.geojson",
            f"../uploads/{self.project_b}/data.geojson",
            f"../../uploads/{self.project_b}/data.geojson",
        ):
            with self.subTest(source=source):
                self.assertIsNone(_resolve_dataset_for_preflight(app_main.config, source, self.project_a))
                with self.assertRaises(WorkflowExecutionError):
                    worker_common.resolve_dataset_path(workspace, source, project_id=self.project_a)
