...
def test_empty_accept_on_second_review_keeps_lease_and_cannot_skip_implementation(q):
    pass_gate(q)
    finish_discovery(q)
    first = q.claim("expert")["task"]
    q.submit(first["id"], first["token"], dict(**basic(first), verdict="revise", issues=[issue()], evidence=["artifacts/flow.md"]))
    reply = q.claim("teacher")["task"]
    q.submit(reply["id"], reply["token"], dict(**basic(reply), accepted=False))
    second = q.claim("expert")["task"]
    assert second["payload"]["discussion"] == 2
    with pytest.raises(Conflict, match="empty list"):
        q.submit(second["id"], second["token"], dict(**basic(second), verdict="accept", issues=[], evidence=["artifacts/flow.md"]))
    assert q.status()["batches"][0]["round"] == 1
    assert q.claim("teacher")["task"] is None
    q.submit(second["id"], second["token"], dict(**basic(second), verdict="accept", issues=[issue()], evidence=["artifacts/flow.md"]))
    reply = q.claim("teacher")["task"]
    q.submit(reply["id"], reply["token"], dict(**basic(reply), accepted=True, issues=[issue()]))
    assert q.claim("expert")["task"]["phase"] == "implement"


def test_agreement_cannot_silently_replace_reviewed_issues(q):
    pass_gate(q)
    finish_discovery(q)
    review = q.claim("expert")["task"]
    q.submit(review["id"], review["token"], dict(**basic(review), verdict="accept", issues=[issue()], evidence=["artifacts/flow.md"]))
    reply = q.claim("teacher")["task"]
    with pytest.raises(Conflict, match="unchanged"):
        q.submit(reply["id"], reply["token"], dict(**basic(reply), accepted=True, issues=[{**issue(), "proposal": "different work"}]))
    assert all(t["phase"] != "implement" for t in q.status()["tasks"])
...
def test_preview_cannot_inherit_production_storage(tmp_path, monkeypatch):
    from scripts.teaching_collab.preview import preview_environment
    production = tmp_path / "production"
    worktree = tmp_path / "worktree"
    monkeypatch.setenv("WEBGIS_AI_DATA_DIR", str(production))
    monkeypatch.setenv("WEBGIS_AI_AUTH_DB", str(production / "auth.db"))
    env = preview_environment(worktree, "http://127.0.0.1:5197", 19079)
    preview_data = Path(env["WEBGIS_AI_DATA_DIR"])
    assert preview_data.is_relative_to(worktree)
    assert Path(env["WEBGIS_AI_AUTH_DB"]).is_relative_to(preview_data)
    assert not preview_data.is_relative_to(worktree / "backend" / "data")
