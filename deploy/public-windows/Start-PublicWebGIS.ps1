...
# Bind the launched instance to the same data and build that release.json records.
# Do not inherit an unrelated preview's data directory, auth DB or stale SHA.
$env:WEBGIS_AI_DATA_DIR = Join-Path $repoRoot "backend\data"
$env:WEBGIS_AI_AUTH_DB = Join-Path $env:WEBGIS_AI_DATA_DIR "auth\auth.db"
$gitCommit = ""
$gitBranch = ""
...
$env:WEBGIS_AI_BUILD_SHA = [string]$gitCommit
