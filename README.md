...
- 新项目在 3D 和 2D 中均默认使用高德影像，支持切换高德标准、高德浅灰和兼容 XYZ 底图；已有项目保留已保存的选择。
...
主页左上角及登录页右上角的太阳/月亮按钮可切换深色、浅色模式。首次访问默认浅色，已有选择会保存在当前浏览器，刷新后仍生效；切换不会重置地图或课堂状态。新增的按钮和面板动效会遵循系统的“减少动态效果”设置。
