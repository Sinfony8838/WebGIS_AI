...
type Props = {
  ...
  /** Active thematic layer ids (see lib/globeThemes.ts). */
  themeIds?: string[];
...
export const Map3DGlobe = forwardRef<Map3DGlobeHandle, Props>(function Map3DGlobe(
  ...
  useEffect(() => {
      ...
      viewer = new Cesium.Viewer(containerRef.current, {
        ...
        contextOptions: {
          webgl: {
            ...
            // Allow canvas readback so classroom snapshot export can
            // include the 3D globe frame.
    ...
    // Light the scene from the camera so thematic 3D primitives (population
    // columns, extruded polygons) stay readable regardless of real-world sun
    // position — otherwise a class running at night sees black columns.
    const cameraLight = new Cesium.DirectionalLight({
      direction: viewer.camera.directionWC
    });
    viewer.scene.light = cameraLight;
    const syncLight = () => {
      cameraLight.direction = Cesium.Cartesian3.clone(
        viewer.camera.directionWC,
        cameraLight.direction as Cesium.Cartesian3
      );
    };
    viewer.scene.preRender.addEventListener(syncLight);

    // Thematic teaching layers (population columns, Hu line, etc.).
    themeManagerRef.current = new GlobeThemeManager(viewer, (themeId, message) => {
      callbacksRef.current.onThemeError?.(themeId, message);
    });
    ...
    // Hover tooltip over thematic entities. Throttled to one pick per rAF
    // and skipped entirely while no themes are active.
    let tooltipRafId: number | null = null;
    screenHandler.setInputAction((movement: Cesium.ScreenSpaceEventHandler.MotionEvent) => {
      if (tooltipRafId !== null) {
        return;
      }
      const endPosition = Cesium.Cartesian2.clone(movement.endPosition);
      tooltipRafId = window.requestAnimationFrame(() => {
        ...
        let tooltip = null;
        try {
          tooltip = getEntityTooltip(viewer.scene.pick(endPosition));
        } catch {
          tooltip = null;
        }
        if (!tooltip) {
  ...
  // Sync active thematic layers with the manager.
  useEffect(() => {
    const manager = themeManagerRef.current;
    const ids = themeIds ?? [];
    themesActiveRef.current = ids.length > 0;
    if (!manager) {
      return;
    }
    manager.syncThemes(ids);
    if (!ids.length && tooltipRef.current) {
  ...
  useImperativeHandle(
    ref,
    (): Map3DGlobeHandle => ({
      flyTo: (lon, lat, altitudeMeters, durationSeconds = 1.2, pitchDeg = -90) => {
        const viewer = viewerRef.current;
        if (!viewer) {
          return;
        }
