    ...
    def submit(self, task_id, token, result):
        with self.transaction() as con:
            ...
            if phase == "probe":
                ...
                if result["verdict"] == "accept" and not result["issues"]:
                    raise Conflict("Accepted review must include the reviewed issues; an empty list cannot authorize implementation")
                if not artifacts:
                ...
                if result["accepted"] and "issues" in result:
                    self._issues(result["issues"], config)
                    if sorted(map(encoded, result["issues"])) != sorted(map(encoded, payload["issues"])):
                        raise Conflict("Agreement must confirm the reviewed issues unchanged; request another review to revise them")
