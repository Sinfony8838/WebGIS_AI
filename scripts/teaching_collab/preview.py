...
def preview_environment(root, preview_url, back_port):
    # Pin both storage overrides even when the coordinator inherited the
    # operator's production environment. Keep preview state out of test data.
    data_root = Path(root).resolve() / ".claude" / "collab-preview-data"
    env = os.environ.copy()
    env.update(WEBGIS_AI_HOST="127.0.0.1", WEBGIS_AI_PORT=str(back_port),
               WEBGIS_AI_DATA_DIR=str(data_root),
               WEBGIS_AI_AUTH_MODE="users", WEBGIS_AI_AUTH_DB=str(data_root / "auth" / "collab.db"),
               WEBGIS_AI_CORS_ALLOW_ORIGINS=preview_url,
...
class Preview:
    ...
    def ensure(self, batch):
        ...
        try:
            ...
            env = preview_environment(root, config["preview_url"], back_port)
