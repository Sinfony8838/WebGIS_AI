...
`verdict: accept` 必须附带非空的完整问题列表，不能用空数组表示“上一轮全部同意”；否则提交会被拒绝，保留当前租约供修正。教师同意时若同时提交 `issues`，其内容必须与已评审列表一致；调整方案应先拒绝并进入下一次评审，不能在同意阶段偷偷更换实现范围。
...
六项探针通过后，协调器才启动指定工作树的预览：前端5197、后端19079，仅绑定127.0.0.1，认证保持 users。数据根固定为该工作树的 `.claude/collab-preview-data`，数据库在其 `auth/collab.db`，覆盖继承的生产存储环境变量。不复制真实用户数据库或旧 runtime。首次管理员初始化由用户在隔离预览完成；受登录阻挡时暂停，不从旧会话寻找凭据。

后端测试仍使用项目的标准隔离保护。若重跑时检测到 `backend/data` 已有实例标记，先由负责人核实是否全为该工作树测试生成的数据，再在同工作树内归档；不要删除生产数据、预览历史，也不要自动设置绕过变量。检测失败会保留日志并暂停批次。
