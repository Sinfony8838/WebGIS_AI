"""中文检索评测样本集：119 条，分为调优集(tune)与保留测试集(heldout)。
...
SAMPLES: list[dict] = [
    ...
    _sample("a05", "上海人口密度资料有哪些", "ambiguous", "tune",
            SH_DENSITY_SET + [POP_DENSITY],
            "限定人口密度后，上海三条密度资料与概念卡片均相关（v2：原泛查询改写为可判定问句）"),
    ...
    _sample("x26", "缓冲区分析的英文缩写怎么读", "no_answer", "heldout", [],
            "缓冲区资料讲概念与步骤，不涉及缩写读音"),
    # ---------- v2 追加（合并主线 PR#16 后）：方向相反 / 否定 / 比较 /
    # 发布年 vs 统计年 / 指标混淆 / 主线新增资料。全部位于调优集，
    # 保留测试集（heldout）保持冻结。 ----------
    _sample("t01", "我国人口分布是不是西多东少", "paraphrase", "tune",
            HU_LINE_SET + [CN_DISTRIBUTION, CN_PATTERN_SUMMARY],
            "反向表述属纠错类问题，相关的是讲东密西疏格局及其成因的资料；不把“西多东少”当同义词注入"),
    _sample("t02", "我国人口分布并非东多西少", "paraphrase", "tune",
            HU_LINE_SET + [CN_DISTRIBUTION, CN_PATTERN_SUMMARY],
            "否定句式仍应找到被否定的格局资料以供核对"),
    _sample("t03", "五普和七普哪个资料更新", "paraphrase", "tune", [CENSUS_5, CENSUS_7],
            "比较问句应同时命中两份普查资料"),
    _sample("t04", "上海和全国的人口密度差距大吗", "paraphrase", "tune",
            SH_DENSITY_SET + [POP_DENSITY, "eval_china_density_2000"],
            "比较问句应同时命中上海与全国两类密度资料"),
    _sample("t05", "2025年发布的上海人口报告", "region_year", "tune", ["eval_shanghai_report_pub2025"],
            "按发布年份检索：资料 time 字段为统计年 2020，标题/关键词含 2025年发布"),
    _sample("t06", "上海2020年常住人口资料", "region_year", "tune",
            ["eval_shanghai_report_pub2025", "eval_shanghai_density_2020", "shanghai_qingpu_population_statistics"],
            "统计年 2020 的上海资料；2025 年发布的报告统计口径同为 2020，属相关结果"),
    _sample("t07", "上海2020年人口总数排名", "no_answer", "tune", [],
            "语料没有上海人口总量资料，也没有任何排名类资料；密度图与普查报告不能冒充总量排名"),
    _sample("t15", "全国2020年人口密度是多少", "region_year", "tune", [POP_DENSITY],
            "指标为密度且统计年为 2020：概念卡片应命中；数量统计表不得冒充密度，2000 年密度图年份冲突不返回"),
    _sample("t08", "上海老年人口比例资料", "no_answer", "tune", [],
            "语料没有老年人口指标资料；人口密度/数量不得冒充比例指标"),
    _sample("t09", "青浦区2020年常住人口", "region_year", "tune", ["shanghai_qingpu_population_statistics"],
            "主线新增：青浦区街镇常住人口（2010—2020），time 含 2020"),
    _sample("t10", "杨浦区各街道常住人口", "region_year", "tune", ["shanghai_yangpu_population_statistics"],
            "主线新增：杨浦区各街道常住人口（2020）"),
    _sample("t11", "年轻环是什么", "natural", "tune", ["kb_shanghai_young_ring_commuting"],
            "主线新增：年轻环与上海职住关系（2017）"),
    _sample("t12", "青浦区某个小区的常住人口", "no_answer", "tune", [],
            "区级街镇汇总资料不是街区级证据，更不是小区级证据"),
    _sample("t13", "我国人口分布西多东少的原因", "paraphrase", "tune",
            HU_LINE_SET + [CN_DISTRIBUTION, CN_PATTERN_SUMMARY],
            "错误前提的成因问句仍应命中讲东密西疏成因的资料以便纠正"),
    _sample("t14", "青藏高原的河谷人口都很密集吗", "natural", "tune", ["tibetan_valley_settlements"],
            "主线新增 geo 单元明确反对一概而论，是该问题的直接资料"),
]
